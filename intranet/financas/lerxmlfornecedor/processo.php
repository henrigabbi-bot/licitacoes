...
for ($i=0; $i < $numFile ; $i++) { 
	...
	foreach($arquivo->NFe as $key => $xml){
		...
		$query2 = mysql_query("SELECT * FROM nfentrada WHERE chavedeacesso = '$chave'"); 
		$consulta2 = mysql_num_rows($query2);

		if ($consulta2 == 0) {						
			$sql="INSERT INTO nfentrada (chavedeacesso,numeronf,data,valornf,cnpj) VALUES ('$chave','$NUMERO','$data','$VALOR_TOT','$CNPJ_EMIT')";		
		//echo $sql;
			if(mysql_query($sql)) {		
				$continsercao++;					
			}
